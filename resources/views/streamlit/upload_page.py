...
def render(user_id: int, role: str) -> None:
    ...
    """
    st.markdown("<h2 style='display: flex; align-items: center; gap: 10px;'><i class='fa-solid fa-file-import'></i> Importer des données</h2>", unsafe_allow_html=True)
    st.markdown("---")
    ...
    if st.button("📤 Importer et traiter", disabled=uploaded_file is None):
        ...
        try:
            ...
            if quality_report:
                ...
                with col1:
                    render_metric_card("Lignes finales", str(quality_report.get("lignes_finales", 0)), "fa-solid fa-list-ol")
                with col2:
                    render_metric_card("Colonnes finales", str(quality_report.get("colonnes_finales", 0)), "fa-solid fa-columns")
                with col3:
                    render_metric_card("Doublons supprimés", str(quality_report.get("doublons_supprimes", 0)), "fa-solid fa-clone")
                with col4:
                    render_metric_card("Valeurs nulles remplies", str(quality_report.get("valeurs_nulles_remplies", 0)), "fa-solid fa-band-aid")
