...
def render(user_id: int, role: str) -> None:
    ...
    """
    st.markdown("<h2 style='display: flex; align-items: center; gap: 10px;'><i class='fa-solid fa-magnifying-glass-chart'></i> Analyses</h2>", unsafe_allow_html=True)
    st.markdown("---")
...
def _display_results(analytics: dict, anomalies: dict, dataset_id: int) -> None:
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["📊 Statistiques", "📉 Visualisations", "⚠️ Anomalies", "🔗 Corrélations", "🤖 Insights IA"]
    )
...
def _render_statistics_tab(analytics: dict) -> None:
    ...
    if kpis:
        ...
        with col1:
            render_metric_card("Total Lignes", str(kpis.get("total_rows", "N/A")), "fa-solid fa-list")
        with col2:
            render_metric_card("Total Colonnes", str(kpis.get("total_columns", "N/A")), "fa-solid fa-table-columns")
        with col3:
            render_metric_card("Complétude", f"{kpis.get('completeness_rate', 0):.1f}%", "fa-solid fa-check-double")
        with col4:
            render_metric_card("Mémoire", f"{kpis.get('memory_usage_mb', 0):.2f} Mo", "fa-solid fa-microchip")
...
def _render_anomalies_tab(anomalies: dict) -> None:
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        render_metric_card("Total Anomalies", str(total), "fa-solid fa-circle-exclamation")
    with col2:
        render_metric_card("Z-Score", str(resume.get("zscore_count", 0)), "fa-solid fa-chart-bar")
    with col3:
        render_metric_card("IQR", str(resume.get("iqr_count", 0)), "fa-solid fa-magnifying-glass-chart")
    with col4:
        render_metric_card("Isolation Forest", str(resume.get("isolation_count", 0)), "fa-solid fa-tree")
...
def _render_correlations_tab(analytics: dict, dataset_id: int) -> None:
    ...
    if not correlations or correlations.get("message"):
        st.info(correlations.get("message", "Données de corrélation non disponibles."))
        return

    # Distribution summary
    distribution = correlations.get("distribution", {})
    if distribution:
        st.markdown("#### Distribution des forces de corrélation")
        cols = st.columns(5)
    ...
    if multicollinearite:
        st.markdown("#### ⚠️ Alertes de multicolinéarité")
        st.warning(
            f"{len(multicollinearite)} paire(s) avec corrélation ≥ 0.9 détectée(s). "
            "Cela peut poser problème pour les modèles de machine learning."
        )
    ...
    # Interactive heatmap
    st.markdown("#### 🗺️ Matrice de corrélation interactive")
    from app.Repositories.dataset_repository import DatasetRepository
    _ds_repo = DatasetRepository()
    dataset = _ds_repo.find_by_id(dataset_id)
    if dataset:
